...
Using the above variables, we define operation values as described below.  It should be noted that the $\alpha_5$ term is multiplied by the [opcode value](../stack/op_constraints.md#high-degree-operations) of the respective control block initializer.
...
$$
u_{join} = f_{join} \cdot (h_{init8} + \alpha_5 \cdot 90) \text{ | degree} = 7
$$
...
$$
u_{split} = f_{split} \cdot (h_{init8} + \alpha_5 \cdot 92) \text{ | degree} = 7
$$

When `LOOP` operation is executed, a new hasher is initialized and contents of $h_0, ..., h_3$ are absorbed into the hasher. The remaining hasher rate elements ($h_4, ..., h_7$) are padded with $0$ and as such the $\alpha_4$ term is set to $0$:

$$
u_{loop} = f_{loop} \cdot (h_{init8} + \alpha_5 \cdot 94) \text{ | degree} = 7
$$
...
when `CALL` operation is executed, a new hasher is initialized and the contents of $h_0, ..., h_3$ are absorbed into the hasher. The remaining hasher rate elements ($h_4, ..., h_7$) are padded with $0$ and as such the $\alpha_4$ term is set to $0$:

$$
u_{call} = f_{call} \cdot (h_{init8} + \alpha_5 \cdot 108) \text { | degree} = 5
$$

when `SYSCALL` operation is executed, a new hasher is initialized and the contents of $h_0, ..., h_3$ are absorbed into the hasher. The remaining hasher rate elements ($h_4, ..., h_7$) are padded with $0$ and as such the $\alpha_4$ term is set to $0$:

$$
u_{syscall} = f_{syscall} \cdot (h_{init8} + \alpha_5 \cdot 104) \text { | degree} = 5
$$
...
> $$
p_0' \cdot (u_{join} + u_{split} + u_{loop} + u_{span} + u_{respan} + u_{call} + u_{syscall} + u_{end} + \\
1 - (f_{join} + f_{split} + f_{loop} + f_{span} + f_{respan} + f_{call} + f_{syscall} + f_{end})) = p_0
$$
